...
class OrcidDataSyncReader(BaseReader):
    ...
    def _fetch_orcid_data(self, flask_app, orcid_to_sync, bucket):
        ...
        flask_app.logger.debug(f"Fetching ORCiD record: {key} from bucket: {bucket}")
        try:
            ...
            flask_app.logger.exception(f"Failed to fetch ORCiD record: {key}")
